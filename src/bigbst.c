...
// Returns bst containing data from the  file
bst_t* parseFile(char *filename) {
	...
	bst_t* bst = parseFileFirstLine(fp);
	while (parseFileDataLine(bst, fp) != PARSE_DONE) {
	}
...
// Parses first line of  file to get:
...
bst_t* parseFileFirstLine(FILE* fp) {
...
// Parses a data entry from  file, inserts it into bst
int parseFileDataLine(bst_t* bst, FILE* fp) {
